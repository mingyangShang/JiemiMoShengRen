...
public class UserInfoActivity extends DefaultActivity {
	...
	protected void initView() {
		...
		label.setImageBitmap(setPictureSize(R.drawable.cardiograph, 0.83f, 0.16f));

		//利用user的信息填写界面内容
		if(user!=null){
			tvUserName.setText(user.getNick());
			tvSign.setText(user.getSign());
			tvSex.setText(user.isFemale()?getString(R.string.hint_sex_female):getString(R.string.hint_sex_male));
	...
	public void addContact(View v){
		if(user!=null){
			addContact(user.getUsername(),getString(R.string.default_add_friend_reason));
		}
	}
